...
optCBs = {
    ...
    'mailAuthFrom': (SMFIP_NOMAIL , SMFIP_NR_MAIL) ,
...
class MilterProtocol(object):
    ...
    def __init__(self , opts=0 , protos=0):
        ...
        self._MACMAP = {
            ...
            SMFIC_AUTHMAIL: self._mailAuthFrom ,
#            SMFIC_AUTHSSF: self._mailAuthSucc ,
    ...
    def _mailFrom(self , cmd , data):
        ...
        if md.has_key('mail_addr'):
            mfrom = md['mail_addr']
        if md.has_key('auth_authen'):
            auth_authen = md['auth_authen']
        if md.has_key('i'):
            self._qid = md['i']
        return self.mailFrom(mfrom, auth_authen, md)


    def _mailAuthFrom(self , cmd , data):
        """
        Parses the i-macro data from the MTA and calls mailAuthFrom()
        with (<auth_authen> <mail_addr>, <auth_ssf>, <cmdDict>)
        """
        md = {}
        if cmd is not None:
            md = dictFromCmd(cmd[2:])
        ...
        if md.has_key('auth_authen'):
            auth_authen = md['auth_authen']
        if md.has_key('mail_addr'):
            mail_addr = md['mail_addr']
        if md.has_key('auth_ssf'):
            auth_ssf = md['auth_ssf']
        if md.has_key('auth_ssf'):
            auth_ssf = md['auth_ssf']
        if md.has_key('i'):
            self._qid = md['i']
        return self.mailAuthFrom(mail_addr, auth_authen, auth_ssf, md)
    ...
    def mailAuthFrom(self , frAddr, frAuthAddr, frSsf , cmdDict):
        """
        This gets the auth_authlen FROM sendmail

        str:frAuthAddr      The autheticated user
        dict:cmdDict        The raw dictionary of items sent by the MTA

        Override this in a subclass.
        """
        self.log('MAIL: %s' % frAddr + ' AMAIL: %s' % frAuthAddr + ' STAT: %s' % frSsf)
        return CONTINUE
